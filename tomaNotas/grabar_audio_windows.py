#!/usr/bin/env python3
"""
grabar_audio_windows.py
-----------------------
Script para Windows que graba audio activado por voz (Voice Activity Detection).
...
Parámetros:
- -u / --umbral: Nivel de amplitud para detectar sonido (default: 500)
- -o / --salida: Nombre del archivo WAV de salida (default: TEST<HH:MM:SS>.wav)
...
try:
    ...
    print("En Windows, instálalas ejecutando:")
...
TASA_MUESTREO  = 44100
CHUNK          = 1024*2
UMBRAL_DEFAULT = 1000   # Ajustar según el ruido de fondo en Windows
...
def listar_dispositivos(audio: pyaudio.PyAudio) -> None:
    """Muestra los dispositivos de audio disponibles en Windows."""
...
def grabar_activado_por_voz(
    ...
    # En Windows, a veces el dispositivo por defecto no es el que esperamos
    # Si no se especifica, intentamos usar el predeterminado del sistema
    try:
        stream = audio.open(
            format=FORMATO,
            channels=CANALES,
        ...
        print("Sugerencia: Ejecuta 'python grabar_audio_windows.py --listar' para ver los IDs.")
        audio.terminate()
        return
...
def main():
    parser = argparse.ArgumentParser(description="Grabación por umbral para Windows.")
